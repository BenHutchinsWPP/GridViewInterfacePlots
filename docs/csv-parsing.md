...
A pool of Web Workers each take an ~8 MiB byte range of the CSV, read it with `file.slice().arrayBuffer()`, and hand it to a **WASM module compiled from C with SIMD128**. The module finds delimiters 16 bytes at a time, parses floats inline without ever creating a JS string, derives each row's area and hour from its own `Date`/`Hour`/`Name` fields, and writes into a compact per-block slab. The main thread blits each slab into the `Float32Array` cube.
...
- **Layout matters more than parser choice.** A naive array-of-objects parse of one case costs 802 MB; the columnar `Float32Array` cube costs 72 MB. The naive version is not slower, it is disqualifying — ten cases would be 8 GB.
...
That is the whole toolchain — `clang --target=wasm32 -O3 -msimd128 -nostdlib`. No Rust, no Emscripten, no wasm-bindgen, no bundler plugin. The module is **6,095 bytes** with no imports, and it is committed to the repo so a deploy needs no wasm toolchain ([D9](decisions.md)).
...
1. **Size static buffers to a BLOCK, never to a case.** Each worker needs its own WASM instance, because linear memory cannot be shared without `SharedArrayBuffer`. Sizing to a case means 8 workers × 160 MiB = **1,280 MiB of parse scratch**, which breaks the memory budget before a single case is retained. The per-block slab brings 8 workers to 256 MiB ([footgun 22](footguns.md)).
2. **Derive area and hour from each row's own fields**, never a global row counter. That is what makes blocks position-independent, and [footguns 17 and 18](footguns.md) require it anyway.

## Correctness

The parser is trusted because of a cell-by-cell comparison against an independent JS reference, not because the charts look right. `test_ingest.mjs` runs it on every change against a synthetic export, and gates on **zero differences beyond one float32 ulp** — reporting the within-one-ulp count as a number rather than hiding it.

`parse_float` accumulates the integer part and adds `frac / scale`, so it rounds twice where `strtod` rounds once. On 8-digit money columns that shows up as a handful of cells below the precision the f32 cube stores. Do not quietly change the gate to an epsilon, and do not "fix" the rounding — see below.
...
- ❌ **The cache-hostile cube write is a non-issue.** Scattering 50 values at 8,760-float stride per row looks pathological; the measured penalty is 1.05×. **Do not reorganize the cube layout for ingest speed.**
...
- **Pool scaling beyond 2 cores is arithmetic, not measured.**
