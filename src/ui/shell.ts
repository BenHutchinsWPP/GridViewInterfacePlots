...
export function createShell(handlers: ShellHandlers): Shell {
  ...
  const caseFilter = byId<HTMLInputElement>('case-filter');
  const casesSummary = byId('cases-summary');
  ...
  /** The case names the filter box currently lists, in rendered order. */
  let shownCases: string[] = [];
  /** Every loaded case name, in rail order. A selection is reported in this
   * order whatever the filter box is showing. */
  let allCases: string[] = [];
  /** The last rendered query's case list. */
  let currentCases: string[] = [];
  /** The last plainly clicked row, the anchor a shift-click ranges from. Held
   * by name and not by index, because the filter box moves rows. */
  let caseAnchor: string | null = null;

  /**
   * Commit a case selection built from what is on screen.
   *
   * The same fold as selectedInterfaces: a case selected under an earlier
   * search is still selected, it is simply not listed, so what the gestures
   * below decide covers the listed cases only and the hidden ones are carried
   * through untouched. With an empty filter box nothing is hidden and this is
   * the plain selection.
   */
  function commitCases(picked: Iterable<string>): void {
    const next = new Set(picked);
    for (const name of currentCases) if (!shownCases.includes(name)) next.add(name);
    handlers.onSelectCases(allCases.filter((name) => next.has(name)));
  }

  // Cases select the way the interface list below them does, because a drop
  ...
  casesList.addEventListener('click', (event) => {
    ...
    const anchor = caseAnchor === null ? -1 : shownCases.indexOf(caseAnchor);
  ...
  caseFilter.addEventListener('input', () => {
    // Same contract as the interface filter: repaint through the app, because
    // the filter box changes what is listed and never what is selected.
    handlers.onQueryChange({});
  });
  ...
  return {
    render(query, state) {
      ...
      allCases = state.cases.map((data) => data.name);
      currentCases = [...query.cases];
      const caseNeedle = caseFilter.value.trim().toLowerCase();
      const matchingCases = caseNeedle
        ? state.cases.filter((data) => data.name.toLowerCase().includes(caseNeedle))
        : state.cases;
      shownCases = matchingCases.map((data) => data.name);
      if (caseAnchor !== null && !shownCases.includes(caseAnchor)) caseAnchor = null;
      if (state.cases.length === 0 || matchingCases.length === 0) {
        const empty = document.createElement('div');
        empty.className = 'case-empty';
        empty.textContent =
          state.cases.length === 0
            ? 'Drop CSV exports anywhere on this window.'
            : 'No case matches this filter.';
        casesList.appendChild(empty);
      }
      matchingCases.forEach((data) => {
        const selected = state.enabled.has(data.name);
        const item = document.createElement('div');
        item.className = 'case-item' + (selected ? ' case-on' : '');
      ...
      // The same accounting the interface control gets, and for the same
      // reason: a selected case the filter is hiding still draws a line, so
      // the count that explains the chart has to be visible without clearing
      // the filter to go and look.
      const casesOffScreen = query.cases.filter((name) => !shownCases.includes(name)).length;
      casesSummary.textContent =
        state.cases.length === 0
          ? ''
          : `${query.cases.length} selected` +
            (casesOffScreen > 0 ? ` · ${casesOffScreen} not shown by this filter` : '') +
            ` · ${matchingCases.length} of ${state.cases.length} listed`;
